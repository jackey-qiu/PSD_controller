...
try:
    ...
    import locate_path
from operationmode.operations import initOperationMode, normalOperationMode, advancedRefillingOperationMode
...
class MyMainWindow(QMainWindow):
    ...
    start_exchange_simple_mode = QtCore.pyqtSignal()
    def __init__(self, parent = None):
        super(MyMainWindow, self).__init__(parent)
        #load GUI ui file made by qt designer
        ui_path = os.path.join(script_path,'psd_gui_debug2.ui')
        ...
        # self.radioButton_init_mode.clicked.connect(self.update_to_init_mode)
        ...
        # self.comboBox_syringe_number.currentTextChanged.connect(self.update_to_normal_mode)
        # self.comboBox_valve_port.currentTextChanged.connect(self.update_to_normal_mode)
        ...
        # self.timer_update_normal_mode.timeout.connect(self.update_volume_waste_reservoir_cell_normal_mode)
    ...
    def init_start_simple(self):
        self.update_to_pre_simple_refilling_mode()
        self.stop_all_timers()
        self.timer_update_simple_pre.start(100)
    ...
    def update_to_normal_mode(self, syringe_no):
        ...
        #self.widget_psd.actived_syringe_normal_mode= 1
    ...
    def fill_init_mode(self):
        self.widget_psd.actived_syringe_motion_init_mode = 'fill'
        # self.init_operation.init_motion()
        self.stop_all_timers()
    ...
    def fill_syringe(self,radioButton_widget,syringe_no):
        ...
        radioButton_widget.setChecked(True)
        # self.normal_operation.init_motion()
        self.stop_all_timers()
    ...
    def start(self):
        ...
        self.widget_psd.filling_status_syringe_1 = True
        self.widget_psd.filling_status_syringe_2 = False
        self.widget_psd.filling_status_syringe_3 = True
        self.widget_psd.filling_status_syringe_4 = False
    ...
    def update_volume_waste_reservoir_cell_normal_mode(self):
        if self.widget_psd.actived_syringe_motion_normal_mode=='fill':
            if self.widget_psd.actived_syringe_valve_connection == 'resevoir':
                ...
                if resevoir_volumn<=0:
                    ...
                    self.widget_psd.resevoir_volumn = resevoir_volumn
            elif self.widget_psd.actived_syringe_valve_connection == 'cell':
                cell_volumn = self.widget_psd.volume_of_electrolyte_in_cell - self.widget_psd.speed_normal_mode/10
                if cell_volumn<=0:
                    self.widget_psd.volume_of_electrolyte_in_cell = 0
                else:
                    self.widget_psd.volume_of_electrolyte_in_cell = cell_volumn
        elif self.widget_psd.actived_syringe_motion_normal_mode=='dispense':
            if self.widget_psd.actived_syringe_valve_connection == 'waste':
    ...
    def update_volume_init_mode_(self):
    ...
    def update_volume_init_mode(self):
        if self.widget_psd.actived_syringe_motion_init_mode == 'fill':
            self.init_operation.exchange_motion(self.widget_psd.actived_pushing_syringe_init_mode)
        elif self.widget_psd.actived_syringe_motion_init_mode == 'dispense':
            self.init_operation.exchange_motion(self.widget_psd.actived_pulling_syringe_init_mode)

    def update_volume_filling_init_mode(self):
        syringe_index_map_tab = {1:'volume_syringe_1', 2:'volume_syringe_2', 3:'volume_syringe_3',4:'volume_syringe_4'}
    ...
    def update_volume_normal_mode(self):
        #let us prevent two disaster situations first
        if self.widget_psd.actived_syringe_motion_normal_mode=='fill':
            if self.widget_psd.actived_syringe_valve_connection == 'waste':
                logging.getLogger().exception('Operation be prohibited: you are not allow to withdraw solution from waste!!!')
                self.tabWidget.setCurrentIndex(2) 
                self.timer_update_normal_mode.stop()
                return
            elif self.widget_psd.actived_syringe_valve_connection == 'cell_inlet':
                logging.getLogger().exception('Operation be prohibited: you are not allow to withdraw solution from inlet of cell!!!')
                self.tabWidget.setCurrentIndex(2) 
                self.timer_update_normal_mode.stop()
                return
        elif self.widget_psd.actived_syringe_motion_normal_mode=='dispense':
            if self.widget_psd.actived_syringe_valve_connection == 'resevoir':
                logging.getLogger().exception('Operation be prohibited: you are not allow to dispense solution to resevoir!!!')
                self.tabWidget.setCurrentIndex(2) 
                self.timer_update_normal_mode.stop()
                return
            elif self.widget_psd.actived_syringe_valve_connection == 'cell_outlet':
                logging.getLogger().exception('Operation be prohibited: you are not allow to dispense solution from outlet of cell!!!')
                self.tabWidget.setCurrentIndex(2) 
                self.timer_update_normal_mode.stop()
                return
        self.normal_operation.start_motion()

    def _update_volume_normal_mode(self):
        #let us prevent two disaster situations first
        if self.widget_psd.actived_syringe_motion_normal_mode=='fill':
            if self.widget_psd.actived_syringe_valve_connection == 'waste':
                logging.getLogger().exception('Operation be prohibited: you are not allow to withdraw solution from waste!!!')
            ...
            if self.widget_psd.actived_syringe_valve_connection == 'resevoir':
                ...
                self.timer_update_normal_mode.stop()
                return
        volume_normal_mode = self.widget_psd.volume_normal_mode - self.widget_psd.speed_normal_mode/10
        if volume_normal_mode < 0:
            self.widget_psd.volume_normal_mode = 0
            speed = volume_normal_mode + self.widget_psd.speed_normal_mode/10
            #self.self.widget_psd.speed_normal_mode 
        else:
            self.widget_psd.volume_normal_mode = volume_normal_mode
            speed = self.widget_psd.speed_normal_mode/10
        syringe_index_map_tab = {1:'volume_syringe_1', 2:'volume_syringe_2', 3:'volume_syringe_3',4:'volume_syringe_4'}

        def _update_volume(syringe_number, speed = speed, timer = self.timer_update_normal_mode, widget = self.widget_psd):
            speed = speed*[-1,1][int(widget.actived_syringe_motion_normal_mode=='fill')]
            # setattr(widget,syringe_index_map_tab[syringe_number],getattr(widget,syringe_index_map_tab[syringe_number])+speed)
            syringe_volume_temp = getattr(widget,syringe_index_map_tab[syringe_number])+speed
            stop_timer = False
            if syringe_volume_temp>widget.syringe_size:
                setattr(widget,syringe_index_map_tab[syringe_number],widget.syringe_size)
                #timer.stop()
                stop_timer = True
            elif syringe_volume_temp<0:
                setattr(widget,syringe_index_map_tab[syringe_number],0)
                #timer.stop()
                stop_timer = True
            else:
                setattr(widget,syringe_index_map_tab[syringe_number],syringe_volume_temp)
            if getattr(widget,'volume_normal_mode') == 0:
                # setattr(widget,'volume_normal_mode',0)
                #timer.stop()
                stop_timer = True
            return stop_timer
        stop_timer = _update_volume(self.widget_psd.actived_syringe_normal_mode)
        #Now update the volume of cell, resevoir or waste if the timer is still acitve
        if self.widget_psd.actived_syringe_motion_normal_mode=='fill':
            if self.widget_psd.actived_syringe_valve_connection == 'resevoir':
                resevoir_volumn = self.widget_psd.resevoir_volumn - speed
                if resevoir_volumn<=0:
                    self.timer_update_normal_mode.stop()
                    self.widget_psd.resevoir_volumn = 0
                else:
                ...
                cell_volumn = self.widget_psd.volume_of_electrolyte_in_cell - speed
                self.widget_psd.volume_of_electrolyte_in_cell = cell_volumn

        elif self.widget_psd.actived_syringe_motion_normal_mode=='dispense':
            if self.widget_psd.actived_syringe_valve_connection == 'waste':
                waste_volumn = self.widget_psd.waste_volumn + speed
                self.widget_psd.waste_volumn = waste_volumn
            elif self.widget_psd.actived_syringe_valve_connection == 'cell_inlet':
                cell_volumn = self.widget_psd.volume_of_electrolyte_in_cell + speed
                self.widget_psd.volume_of_electrolyte_in_cell = cell_volumn

        if self.timer_update_normal_mode.isActive():
            if stop_timer:
                self.timer_update_normal_mode.stop()
    ...
    def update_volume_fill_half_mode(self):
        self.advanced_exchange_operation.premotion()

    def _update_volume_fill_half_mode(self):
    ...
    def update_volume(self):
        ...
        for vol_tag, fill_tag in zip(vol_tags,fill_tags):
            ready_, volume_ = self._update_volume(vol_tag,fill_tag)
            ready.append(ready_)
            real_change_volume.append(volume_)
        if 'ready' in ready and 'not_ready' in ready:
            self.auto_refilling_sychronized_ready = False
        else:
            self.auto_refilling_sychronized_ready = True
        #in this mode, the syringes' connections are known.
        #s1 and s2 either goes to inlet of cell or resevoir; the other two either goes to cell outlet or waste.
        if real_change_volume[0]>=0:
        ...
        elif real_change_volume[1]>=0:
            self.widget_psd.resevoir_volumn = self.widget_psd.resevoir_volumn - real_change_volume[1]
            self.exchange_volume = self.exchange_volume + real_change_volume[1]
        if real_change_volume[2]<=0:
            self.widget_psd.waste_volumn = self.widget_psd.waste_volumn - real_change_volume[2]
        elif real_change_volume[3]<=0:
            self.widget_psd.waste_volumn = self.widget_psd.waste_volumn - real_change_volume[3]
